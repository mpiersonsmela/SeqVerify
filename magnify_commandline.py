...
import argparse
import os
from magnify_functions import *
from transgene_histogram import *
...
os.system(f"samtools view -h magnify_temp_{output_name}/magnify_{output_name}_markers.sam |gawk '{{if ((($3 ~ /^({markers})/)&&($7 ~ /chr([0-9]+|[XYM])/))||($1 ~ /^@/)) print $0}};' > magnify_temp_{output_name}/magnify_{output_name}_markers_diff_chr.sam")
...
os.system(f'samtools view -H magnify_{output_name}/magnify_{output_name}_markers.bam > magnify_temp_{output_name}/magnify_{output_name}_markers_header.sam')

bed_file = region_bed(f"magnify_temp_{output_name}/magnify_{output_name}_markers_header.sam",marker_list)

os.system(f'samtools depth -b {bed_file} magnify_{output_name}/magnify_{output_name}_markers.bam > magnify_temp_{output_name}/magnify_{output_name}_markers_coverage.cov')
